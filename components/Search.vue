<script setup lang="ts">
    ...
    let controller: AbortController, signal: AbortController['signal'];
    ...
    const updateList = async () => {
        ...
        try {
            const response = await search('movie', searchTerm.value, { signal });
            results.value = response.results;
        }
        catch (err) {
            if (!err.toString().includes('user aborted')) console.error(err);
        }
    };

    watch(searchTerm, updateList);
</script>
    ...
    <div class="search">
        ...
        <ul v-if="results?.length && open" class="search__list">
            <li v-for="result in results" :key="result.id" class="search__item">
                ...
                <p><NuxtLink class="transparent-link block-link search__link" :to="'/movie/' + result.id" @click.native="open = false">{{ result.title }} <span v-if="result.release_date" class="year">({{ result.release_date.split('-')[0] }})</span></NuxtLink></p>
            </li>
        </ul>
    </div>
</template>
